...
export async function exportAppData(
  ...
  const row = await db
    ...
    'users', json((SELECT json_group_array(json_object('id',id,'role',role,'password',password,
      'profile',profile,'first_login',first_login,'blocked_until',blocked_until)) FROM (SELECT * FROM users ORDER BY id))),
    ...
    ) AS payload FROM users WHERE id=? AND role='admin' AND password=?`)
...
export async function restoreAppData(
  ...
  const columns =
    "json_extract(value,'$.id'),json_extract(value,'$.role'),json_extract(value,'$.password'),json_extract(value,'$.profile'),json_extract(value,'$.first_login'),json_extract(value,'$.blocked_until')";
  const result = await db.batch([
    db
      ...
      AND NOT EXISTS (SELECT 1 FROM bookings)
      AND NOT EXISTS (SELECT 1 FROM users WHERE role!='admin')
      AND EXISTS (SELECT 1 FROM users WHERE id=? AND role='admin' AND password=?)`)
      .bind(token, JSON.stringify(defaultSettings), adminId, verifiedHash),
    db.prepare(`DELETE FROM sessions WHERE ${guard}`).bind(token),
    // Install the incoming admins first so the last-admin guards remain enabled throughout.
    db
      .prepare(`INSERT INTO users(id,role,password,profile,first_login,blocked_until)
      SELECT ${columns} FROM json_each(?) WHERE json_extract(value,'$.role')='admin' AND ${guard}
      ON CONFLICT(id) DO UPDATE SET role=excluded.role,password=excluded.password,profile=excluded.profile,
        first_login=excluded.first_login,blocked_until=excluded.blocked_until`)
      .bind(users, token),
    db
      .prepare(`DELETE FROM users WHERE id NOT IN
      (SELECT json_extract(value,'$.id') FROM json_each(?) WHERE json_extract(value,'$.role')='admin') AND ${guard}`)
      .bind(users, token),
    db
      .prepare(`INSERT INTO users(id,role,password,profile,first_login,blocked_until)
      SELECT ${columns} FROM json_each(?) WHERE json_extract(value,'$.role')!='admin' AND ${guard}`)
